...
| Frontend | React (Vite) |
| Database | 로컬 Docker MySQL · 테스트 H2 → AWS RDS |
| LLM | 외부 LLM API |
...
사전 요구: **JDK 21** · **Docker** (Gradle은 포함된 Wrapper 사용)

**1) 로컬 DB(MySQL) 기동** — 저장소 루트에서:

```bash
docker compose up -d
```

- MySQL 8이 `localhost:3306`에 뜹니다(DB `aiinterviewer`, user·pw `aiinterviewer`). 데이터는 볼륨에 **영속**.
- **DataGrip 등으로 관찰**: 호스트 `localhost` · 포트 `3306` · DB `aiinterviewer` · 계정 `aiinterviewer/aiinterviewer`.
- 중지: `docker compose down` (데이터 유지) / 초기화: `docker compose down -v`.

**2) LLM 키 설정 (선택)** — 꼬리질문 기능을 쓰려면 `backend/.env`에 키를 넣습니다:
...
- **키가 없어도 앱은 정상 실행**됩니다. 이 경우 꼬리질문 호출만 `503(LLM_NOT_CONFIGURED)`로 비활성.
- 키 발급: Google AI Studio (Gemini API, 무료 티어). IntelliJ로 실행해도 `.env` 자동 로딩(spring-dotenv).
...
- DB 관찰: 위 DataGrip 접속 정보(재시작해도 데이터 유지). 기동 시 seed 9개 카테고리·193문항 적재.
